...
TypeSystemClang::TypeSystemClang(llvm::StringRef name,
  ...
  LogCreation(clang::Language::Unknown);
}
...
void TypeSystemClang::LogCreation(clang::Language language) const {
  if (auto *log = GetLog(LLDBLog::Expressions))
    LLDB_LOG(log, "Created new TypeSystem for (ASTContext*){0:x} '{1}' for target language '{2}'",
             &getASTContext(), getDisplayName(), clang::languageToString(language));
}
