...
template <typename DeclTy> void CheckRedeclarationChain(DeclTy *record) {
  ASSERT_TRUE(record);

  auto *def = record->getDefinition();
  ASSERT_TRUE(def);

  // Ensure that we created two distinct declarations that declare the same
  // type.
...
TEST_F(TestClangRedeclarations, RedeclareCppClass) {
  ...
  auto *record =
      llvm::cast_or_null<CXXRecordDecl>(ClangUtil::GetAsTagDecl(class_type));
  ASSERT_TRUE(record);
...
TEST_F(TestClangRedeclarations, RedeclareEnum) {
  ...
  if (auto err = m_ast->CreateRedeclaration(enum_type))
    llvm::handleAllErrors(std::move(err));
...
TEST_F(TestClangRedeclarations, NestedDecls) {
  ...
  if (auto err = m_ast->CreateRedeclaration(context_class))
    llvm::handleAllErrors(std::move(err));
...
TEST_F(TestClangRedeclarations, MetadataRedeclaration) {
  ...
  if (auto err = m_ast->CreateRedeclaration(class_with_metadata))
    llvm::handleAllErrors(std::move(err)); // No need for the if-statement...can just be handleAllErrors(CreateRedeclaration())
