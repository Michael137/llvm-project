...
TypeSystemClang::GetBitSize(lldb::opaque_compiler_type_t type,
  ...
  case clang::Type::IncompleteArray: {
    ...
    size_t total_children = std::accumulate(array_info->element_orders.begin(), array_info->element_orders.end(), 1, std::multiplies<uint64_t>());
    const auto maybe_bitsize = GetBitSize(qual_type->getAsArrayTypeUnsafe()->getElementType().getAsOpaquePtr(), exe_scope); // TODO: is element_type here struct Foo? or Foo[]? The latter won't work
    if (!maybe_bitsize)
      return 0;
