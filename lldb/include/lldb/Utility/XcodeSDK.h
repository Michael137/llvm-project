...
class XcodeSDK {
  std::string m_name;
  std::optional<std::string> m_sysroot_path; // TODO: should this be std::optional<FileSpec>?
...
public:
  ...
  XcodeSDK(std::string name, std::string sysroot) : m_name(std::move(name)), m_sysroot_path(std::move(sysroot)) {}
