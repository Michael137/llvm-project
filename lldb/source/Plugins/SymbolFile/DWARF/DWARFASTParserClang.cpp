...
DWARFASTParserClang::ParseArrayType(const DWARFDIE &die,
  ...
  CompilerType array_element_type = element_type->GetForwardCompilerType();// TODO: element_type is Foo because of how multi-dimensional VLAs get represented in DWARF.
                                                                           ...
                                                                           //              Does this account for aligment?
                                                                           //
                                                                           // TODO: Need to make sure we account for alignment of underlying element type when calculating size of the various VLA possibilities.
                                                                           //       We might need to go with the "synthesize intermediate array types" approach to do this properly?
  TypeSystemClang::RequireCompleteType(array_element_type);

  uint64_t array_element_bit_stride = byte_stride * 8 + bit_stride;
  CompilerType clang_type;
  const bool is_incomplete_array = array_info && array_info->is_incomplete;
  if (array_info && array_info->element_orders.size() > 0) {
    ...
    for (auto pos = array_info->element_orders.rbegin(); pos != end; ++pos) {
      ...
      clang_type = m_ast.CreateArrayType(array_element_type, num_elements,
                                         attrs.is_vector, is_incomplete_array);
