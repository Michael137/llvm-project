...
bool FormatEntity::Format(const Entry &entry, Stream &s,
  ...
  case Entry::Type::FunctionName: {
    ...
    if (language_plugin)
      language_plugin_handled = language_plugin->GetFunctionDisplayName(
          sc, exe_ctx, Language::FunctionNameRepresentation::eName, ss, entry.highlight);
  ...
  case Entry::Type::FunctionNameWithArgs: {
    ...
    if (language_plugin)
      language_plugin_handled = language_plugin->GetFunctionDisplayName(
          sc, exe_ctx, Language::FunctionNameRepresentation::eNameWithArgs, ss, entry.highlight);
...
static llvm::Expected<Entry::HighlightSettings> ParseHighlightSettings(const Entry & entry) {
  ...
  if (terminal_code.empty())
    return llvm::createStringError(
        "Invalid color variable '%s' found in: %s",
        color_format.c_str(),
        entry.printf_format.c_str());

  settings.color_variable = std::move(terminal_code);
  settings.color_end = ansi::FormatAnsiTerminalCodes("${ansi.normal}");
