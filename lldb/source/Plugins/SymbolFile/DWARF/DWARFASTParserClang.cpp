...
TypeSP DWARFASTParserClang::ParseTypeFromDWARF(const SymbolContext &sc,
  ...
  if (TypeSystemClang::UseRedeclCompletion()) {
    if (auto * type_sp = dwarf->GetDIEToType().lookup(die.GetDIE()))
      return type_sp->shared_from_this();
...
DWARFASTParserClang::ParseStructureLikeDIE(const SymbolContext &sc,
  ...
  clang::DeclContext *containing_decl_ctx = GetClangDeclContextContainingDIE(decl_die, nullptr);

  const bool should_directly_complete = DirectlyCompleteType(containing_decl_ctx, attrs);
  ...
  TypeSystemClang::TemplateParameterInfos template_param_infos;
  if (!clang_type_was_created && ParseTemplateParameterInfos(def_die, template_param_infos)) {
  ...
  // function prototypes.
  clang::DeclContext *type_decl_ctx =
      TypeSystemClang::UseRedeclCompletion() ? GetClangDeclContextContainingDIE(def_die, nullptr) : TypeSystemClang::GetDeclContextForType(clang_type);
