...
clang::CXXMethodDecl *TypeSystemClang::AddMethodToCXXRecordType(
  ...
  if (name.starts_with("~")) {
    ...
    if (!structor_names.empty()) {
      ...
      cxx_dtor_decl->addAttr(clang::StructorMangledNamesAttr::CreateImplicit(getASTContext(), names.data(), names.size()));
    }
    ...
    if (!structor_names.empty()) {
      auto names = llvm::to_vector_of<llvm::StringRef>(structor_names);
      cxx_ctor_decl->addAttr(clang::StructorMangledNamesAttr::CreateImplicit(getASTContext(), names.data(), names.size()));
    }
