  ...
  for (auto const &sc : sc_list.SymbolContexts()) {
    if (auto *func = sc.function) {
      ...
      if (spec_die.IsValid() && spec_die == die) {
        ...
        structor_names.push_back(llvm::formatv("{0}:$__lldb_func_{1}:{2}", *maybe_structor_kind, func_die.GetModule().get(), die.GetID()));
      }
    }
  }
  ...
  if (attrs.name) {
    ...
    if (!type_handled) {
      ...
      if (!function_decl) {
        ...
        if (function_decl) {
          auto ident = llvm::formatv("$__lldb_func_{0}:{1}", die.GetModule().get(), die.GetID());
